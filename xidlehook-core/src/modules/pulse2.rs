...
//! if so it lets xidlehook run the next timer command. This
//! is used to implement `--when-audio` in the xidlehook example
//! application.
...
impl fmt::Debug for WhenAudio {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "WhenAudio")
    }
}
impl Drop for WhenAudio {
    fn drop(&mut self) {
